...
export const sendTaskNotification = async (incident, task) => {
  try {
...
export const sendTaskNotifications = async (incident, tasks) => {
  const results = await Promise.all(tasks.map((task) => sendTaskNotification(incident, task)));
  return results;
};
